...
-- [[Lua Name: HVH Ultra Fakelag Boost Lua
       -- Update Creative: Q1ng 
       -- Version: 2.0 Release Recoder Laster Update Time: 2021.2.23
       -- Edition: Q1ng Skeet Lua Edition: Private Personal Use
-- ]]

client.color_log(0, 255, 255, "[gamesense]HVH Fakelag Lua 已加载 Version: 2.0 Last Update Time: 2.23")
client.color_log(0, 255, 255, "[gamesense]HVH Fakelag Lua 已加载 Version: 2.0 Last Update Time: 2.23")
client.color_log(0, 255, 255, "[gamesense]HVH Fakelag Lua 已加载 Version: 2.0 Last Update Time: 2.23")

local fakelag_enabled = ui.new_checkbox("AA", "Fake lag", "Enabled Q1ng Fakelag Release V3")
...
local boost_send_time = ui.new_slider("AA", "Fake lag", "Boost Send Time", 1, 16, 1, true)
...
local enabled_lag_teleport = ui.new_checkbox("AA", "Fake lag", "> On Peek Teleport")
local teleport_key = ui.new_hotkey("AA", "Fake lag", "On Peek Teleport Key", true)
local teleport_mode = ui.new_combobox("AA", "Fake lag", "\n On Peek Teleport Types", {"On Shot AA Based", "Double Tap Based"})
local teleport_indicator = ui.new_checkbox("AA", "Fake lag", "> [Fake Lag Increase] Teleport Indicator")
local teleport_color = ui.new_color_picker("AA", "Fake lag", "\n Teleport Indicator Color", 0, 255, 255, 255)
...
local fakelag_urmaxprocess = ui.reference("MISC", "Settings", "sv_maxusrcmdprocessticks2")
...
local target_send_state = 1
local tickbase_charging = 0
local boost_fakelag_limit = 1
local Q1ng_State = "Q1ng-Default"
local Q1ng_weapon_visible = false
...
local function alternative_send_time(number_value, time)
	if number_value == 0 then
		target_send_state = target_send_state == (time + 1) and 1 or target_send_state + 1
	end
...
local function Q1ng_visible_enemy(ctx)
	...
	Q1ng_weapon_visible = visible and true or false
	stored_target = enemy_entindex
	local enemy_player_number = entity.get_players(true)
end

client.set_event_callback("paint", Q1ng_visible_enemy)

client.set_event_callback("weapon_fire", function(e)
	local localplayer = entity.get_local_player()
	if not entity.is_alive(localplayer) or not client.userid_to_entindex(e.userid) == localplayer then
		return
	end
...
client.set_event_callback("paint", function(e)
	local local_player = entity.get_local_player()
	if not ui.get(fakelag_enabled) or not ui.get(enabled_lag_teleport) or not ui.get(teleport_key) or not ui.get(teleport_indicator) or not entity.is_alive(local_player) then
		return
	end
	...
	if tickbase_shift == nil then
		return
	end

	local tickbase_slider = 0
	local weapons = entity.get_player_weapon(local_player) or 0
	local tickbase = entity.get_prop(local_player, "m_nTickBase") or 0
	local teleport_r, teleport_g, teleport_b, teleport_a = ui.get(teleport_color)
	local fired_shoot = next_attack <= tickbase_shift * globals.tickinterval() - 0.12
	if tickbase ~= nil and weapons ~= nil then
		local curtime = globals.tickinterval() * (tickbase - 16) or 64
		if curtime ~= nil then
			tickbase_slider = entity.get_prop(weapons, "m_flNextPrimaryAttack") == nil and 0 or entity.get_prop(weapons, "m_flNextPrimaryAttack") - curtime or 0
		end
	end

	if not fired_shoot or not ((ui.get(on_shot) and ui.get(on_shot_hotkey)) or (ui.get(double_tap) and ui.get(double_tap_hotkey))) and tickbase_slider <= - 0.000001 then
		teleport_r, teleport_g, teleport_b, teleport_a = 255, 0, 0, 255
	end

	renderer.indicator(teleport_r, teleport_g, teleport_b, teleport_a, "TP")
end)
...
local function status_lag_shift(e)
	...
	if multi_select(ui.get(fakelag_slider_trigger), "Avoid Wall Peek") and can_fire_wall == true and Q1ng_weapon_visible == false and velocity_speed > 5 then
	...
	if multi_select(ui.get(fakelag_slider_trigger), "Avoid Visible Peek") and Q1ng_weapon_visible == true and velocity_speed > 5 then
...
local function boost_fakelag(cmd)
	...
	elseif ui.get(boost_mode) == "Alternative" then
		local send_packet_target_time = not alternative_send_time(cmd.chokedcommands, ui.get(boost_send_time))
		boost_amount = "Maximum"
		...
		if multi_select(ui.get(boost_condition), "Standing") and player_flags == "Standing" then
			boost_limit = ((ui.get(boost_send_time) == 1 and ui.get(lagcomp_break)) or (ui.get(boost_send_time) > 1 and send_packet_target_time)) and boost_send_overlap or ui.get(boost_choke_limit)
...
client.set_event_callback("setup_command", function(cmd)
	local localplayer = entity.get_local_player()
	if not entity.is_alive(localplayer) or not ui.get(fakelag_enabled) or not ui.get(fakelag_trigger_enabled) then
		return
	end
	...
	elseif flags_on_land == true and flags_weapon_switch <= 0 and not in_freeze_time and fired_shoot == false and not flags_climbing_adder then
		Q1ng_State = "Q1ng-On Land"
	elseif flags_climbing_adder and not in_freeze_time then
		Q1ng_State = "Q1ng-Climbing Ldder"
	elseif in_freeze_time then
		Q1ng_State = "Q1ng-Freeze Time"
	end

	local trigger_always_packet = true
	local trigger_send_packet = true
	if multi_select(ui.get(fakelag_send_packet_trigger), "On Standing") and Q1ng_State == "Q1ng-Default" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "While Moving") and Q1ng_State == "Q1ng-Running" or multi_select(ui.get(fakelag_send_packet_trigger), "While Moving") and Q1ng_State == "Q1ng-Slow Motion" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Duck") and Q1ng_State == "Q1ng-Crouching" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On High Speed") and Q1ng_State == "Q1ng-High Speed" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Jump") and Q1ng_State == "Q1ng-Air" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Exploit") and Q1ng_State == "Q1ng-Exploit" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "Weapon Fired") and Q1ng_State == "Q1ng-Fired" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Use") and Q1ng_State == "Q1ng-In Use" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Freezetime") and Q1ng_State == "Q1ng-Freeze Time" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "Weapon Switch") and Q1ng_State == "Q1ng-Weapon Switch" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "Weapon Reload") and Q1ng_State == "Q1ng-Weapon Reload" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "On Land") and Q1ng_State == "Q1ng-On Land" then
		trigger_always_packet = false
	end

	if multi_select(ui.get(fakelag_send_packet_trigger), "While Climbing Ladder") and Q1ng_State == "Q1ng-Climbing Ldder" then
		trigger_always_packet = false
	end
...
local function choke_cycle(cmd)
	...
	if ui.get(fakelag_enabled) and ui.get(enabled_lag_teleport) and ui.get(teleport_key) then
		if ui.get(teleport_mode) == "On Shot AA Based" then
			ui.set(on_shot, true)
			ui.set(double_tap_hotkey, "Toggle")
			ui.set(on_shot_hotkey, (player_will_peek(e) or Q1ng_weapon_visible) and "Toggle" or "Always on")
		elseif ui.get(teleport_mode) == "Double Tap Based" then
			ui.set(double_tap, true)
			ui.set(on_shot_hotkey, "Toggle")
			ui.set(double_tap_hotkey, (player_will_peek(e) or Q1ng_weapon_visible) and "Toggle" or "Always on")
		end

	elseif not ui.get(fakelag_enabled) or not ui.get(enabled_lag_teleport) or not ui.get(teleport_key) then
		ui.set(on_shot_hotkey, "Toggle")
		ui.set(double_tap_hotkey, "Toggle")
	end
...
local set_cache = function(self)
	...
	if not entity.is_alive(localplayer) or not ui.get(fakelag_enabled) or ui.get(ref_fakeduck) or weapon == nil or multi_select(weapons_disabled, entity.get_classname(weapon)) or aerial_flags or not muti_exec_shooting then
		return
	end
...
client.set_event_callback("paint_ui",function(e)
	...
	ui.set_visible(enabled_lag_teleport, ui.get(fakelag_enabled))
	ui.set_visible(fakelag_trigger_enabled, ui.get(fakelag_enabled))
	ui.set_visible(teleport_key, ui.get(fakelag_enabled) and ui.get(enabled_lag_teleport))
	ui.set_visible(teleport_mode, ui.get(fakelag_enabled) and ui.get(enabled_lag_teleport))
	ui.set_visible(teleport_indicator, ui.get(fakelag_enabled) and ui.get(enabled_lag_teleport))
	...
	ui.set_visible(teleport_color, ui.get(fakelag_enabled) and ui.get(enabled_lag_teleport) and ui.get(teleport_indicator))
	...
	ui.set_visible(boost_send_time, ui.get(fakelag_enabled) and ui.get(fakelag_types) == "Boost Fakelag" and ui.get(boost_mode) == "Alternative" and #ui.get(boost_condition) >= 1)
...
client.delay_call(0.5, function(e)
	client.color_log(0, 125, 255, "[gamesense]HVH Fakelag Lua 加载完毕 Version: 2.0 Last Update Time: 2.23")
end)