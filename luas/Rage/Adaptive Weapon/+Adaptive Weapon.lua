...
local function import_weapon()
    local settings = json.parse(base64.decode(clipboard.get(), "base64"))

    for key, value in pairs(config_names) do
    ...
    print("Imported adaptive config from clipboard!")
end

local function export_weapon()
    local settings = {}
    for key, value in pairs(config_names) do
    ...
    clipboard.set(base64.encode(json.stringify(settings), "base64"))
    print("Exported adaptive config to clipboard!")
end

local import_weaponcfg = ui.new_button("RAGE", "aimbot", "Import Adaptive Config",
    import_weapon)
local export_weaponcfg = ui.new_button("RAGE", "aimbot", "Export Adaptive Config",
    export_weapon)
...
local function handle_menu()
    ...
    ui_set_visible(export_weaponcfg,enabled)
    ui_set_visible(import_weaponcfg,enabled)
...
local function on_setup_command(c)
    ...
    weapon = entity_get_prop(entity_get_player_weapon(local_player), "m_iItemDefinitionIndex"), 0xFFFF

    if weapon ~= nil then
        weapon_id = bit_band(weapon)
    else
        return
    end
