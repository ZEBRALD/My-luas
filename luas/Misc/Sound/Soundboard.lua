...
local function toggleMic(on)
	...
	local loopback_val = loopback:get() and 1 or 0;
	cvar.voice_loopback:set_int(on and loopback_val or 0);
	cvar.voice_inputfromfile:set_int(on and 1 or 0);
	client.exec((on and '+' or '-') .. 'voicerecord');
end

-- 提取核心逻辑函数
local function play_logic()
    if (oldLength > 0) then
...
local function stop_logic()
    toggleMic(false)
end

-- 原始播放/停止按钮
ui.new_button("LUA", "A", "Play", play_logic);
ui.new_button("LUA", "A", "Stop", stop_logic);

-- 新增：按键绑定
local play_hotkey = ui.new_hotkey("LUA", "A", "Hotkey: Play", false)
local stop_hotkey = ui.new_hotkey("LUA", "A", "Hotkey: Stop", false)

ui.new_label("LUA", "A", "--------------[End Soundboard]-------------");

-- 用于按键边缘触发检测的变量
local last_play_state = false
local last_stop_state = false
...
client.set_event_callback('paint_ui', function()
    ...
    -- 2. 新增：按键触发检测
    local current_play_state = play_hotkey:get()
    if current_play_state and not last_play_state then
        play_logic()
    end
    last_play_state = current_play_state

    local current_stop_state = stop_hotkey:get()
    if current_stop_state and not last_stop_state then
        stop_logic()
    end
