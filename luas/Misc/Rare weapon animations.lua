...
-- -----------------------------------------------------------------------------
-- 核心修复部分：将栏目名称改为 "Model option"
-- -----------------------------------------------------------------------------
local enabled_reference = ui_new_checkbox("SKINS", "Model options", "Rare weapon animations")
local active_rare_animations_reference = ui_new_multiselect("SKINS", "Model options", "\nActive rare animations", table_map(sequence_overrides, function(sequence_override) return sequence_override.name or sequence_override.weapon.name end))
